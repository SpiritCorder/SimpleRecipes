...
const RecipeForm = ({
  ...
  useEffect(() => {
    if (isEdit && recipeId) {
      // fetch & populate existing recipe data
      const fetchRecipeData = async () => {
        try {
          const response = await axiosPrivate.get(`/recipes/${recipeId}`);
          const recipeData = response.data.data;
          // populate the state with recipe data
          setName(recipeData?.name);
          setDescription(recipeData?.description);
          addIngredient(recipeData?.ingredients);
          setLoading(false);
        } catch (err) {
  ...
  }, [axiosPrivate, navigate, isEdit, recipeId, addIngredient]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        await axiosPrivate.put(`/recipes/${recipeId}`, JSON.stringify(data));
      } else {
        await axiosPrivate.post("/recipes", JSON.stringify(data));
      }
  ...
  if (isEdit && recipeId && loading) {
    return (
      <div className="w-full flex-row-center">
        <svg
          aria-hidden="true"
          className="w-10 h-10 text-gray-200 animate-spin dark:text-red-500 fill-red-500"
          viewBox="0 0 100 101"
          fill="none"
